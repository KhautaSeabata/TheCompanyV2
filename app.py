...
socketio = SocketIO(app, async_mode='threading') # Use threading for simplicity with websockets
...
def process_tick(symbol, price, tick_time):
    ...
    with data_lock:
        ...
        for interval in INTERVALS:
            ...
            # Convert timestamp to ISO string for Lightweight Charts
    ...
    with data_lock:
        for symbol in SYMBOLS:
            ...
            emit('initial_tick_data', {
                'symbol': symbol,
                'data': list(tick_data[symbol]) # Convert deque to list for JSON serialization
            }, namespace='/')
